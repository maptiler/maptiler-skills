# Source: https://docs.maptiler.com/sdk-js/examples/animated-route/

# Display buildings in 3D

This example shows how to animate a path or route on the map based on keyframes or GeoJSON data. Animated line styling and camera following make it ideal for visualizing routes, playback tracks, or timeline-based geographic events.
...
function getElementsByIds(ids) {
    ...
    map.on('load', async function () {
      const response = await fetch("/sdk-js/examples/animated-route/data/seixal-funchal.geojson");
      const pathGeojson = (await response.json());
      ...
      const img = await map.loadImage("/sdk-js/examples/animated-route/data/mt.png");
      ...
      const animatedRouteLayer = new maptilersdk.AnimatedRouteLayer({
        ...
        iterations: 1,
        pathStrokeAnimation: {
          activeColor: ACTIVE_LINE_COLOR,
          inactiveColor: INACTIVE_LINE_COLOR,
        },
        cameraAnimation: {
          follow: true,
          pathSmoothing: {
      ...
      const prologueAnimation = new maptilersdk.MaptilerAnimation({
        keyframes: getPrologueKeyframes(),
        duration: 10000,
        iterations: 1,
      });

      const epilogueAnimation = new maptilersdk.MaptilerAnimation({
        keyframes: getEpilogueKeyframes(),
        duration: 10000,
        iterations: 1,
      });

      let activeAnimation = prologueAnimation;

      const { coordinates: pathCoordinates } = pathGeojson.features[0].geometry;

      const markerPathFeature = {
        type: "Feature",
        geometry: {
          type: "LineString",
          ...
          coordinates: maptilersdk.resamplePath(pathCoordinates, 10),
        },
        properties: {},
      };

      const markerMovementKeyframes = maptilersdk.parseGeoJSONFeatureToKeyframes(markerPathFeature, {
        pathSmoothing: false,
      });

      // now we create the marker movement animation
      const markerMovementAnimation = new maptilersdk.MaptilerAnimation({
      ...
      const hikerMarker = new maptilersdk.Marker({
        element,
        offset: [0, -20],
        subpixelPositioning: true,
      });

      hikerMarker.setLngLat(new maptilersdk.LngLat(pathCoordinates[0][0], pathCoordinates[0][1]));
      ...
      markerMovementAnimation.addEventListener("timeupdate", (e) => {
        ...
        hikerMarker.setLngLat(new maptilersdk.LngLat(lng, lat));

        poiMarkers.forEach((marker) => {
          const markerPosition = marker.getLngLat();
          const distance = markerPosition.distanceTo(new maptilersdk.LngLat(lng, lat));
          if (distance < 2000) {
            marker.setOpacity("1");
          } else {
            marker.setOpacity("0");
      ...
      playButton?.addEventListener("click", () => {
        activeAnimation.play();
        if (activeAnimation === animatedRouteLayer.animationInstance) {
          markerMovementAnimation.play();
        }
      });
  ...
  `;
    ...
    function createPOIMarkerElement(name, img) {
      const id = name.replace(/ /g, "-").toLowerCase();
      const html = `
    <div id="marker-${id}" style="width: 2.5rem; transform: rotateZ(-45deg); height: 2.5rem; border-radius: 50% 50% 50% 0%; border: 2px solid ${BORDER_COLOR}; background: green; color: ${BORDER_COLOR}; display: flex; justify-content: center; align-items: center;">
  ...
  `;
    ...
    function createPOIMarkers(map) {
      return routePoints.map((point) => {
        ...
        marker.setLngLat(new maptilersdk.LngLat(point.position[0], point.position[1]));
        marker.addTo(map);
        return marker;
      });
    }
