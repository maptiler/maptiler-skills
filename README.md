...
Install straight from the repository with a single cross-platform command:

```bash
git clone https://github.com/RadekDuba/test-skill.git test-skill-temp && mkdir -p ~/.gemini/skills/ && cp -r test-skill-temp/skills/maptiler ~/.gemini/skills/ && rm -Force -Recurse test-skill-temp
```
